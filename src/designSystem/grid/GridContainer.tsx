...
export const GridContainer = styled.div<GridContainerProps>`
  ...
  grid-template-columns: ${(props) => `repeat(${props.theme.grid.small.columns}, 1fr)`};
  margin: ${({ theme, $isRootParent }) => ($isRootParent ? `0 ${theme.grid.small.margin}px` : '0')};
  ...
  ${({ theme, $isRootParent }) => css`
    @media screen and (min-width: ${`${theme.breakpoints.medium}px`}) {
      ...
      max-width: ${$isRootParent ? `calc(100% - ${theme.grid.medium.margin * 2}px)` : '100%'};
    ...
    @media screen and (min-width: ${`${theme.breakpoints.large + theme.grid.large.margin * 2}px`}) {
      grid-column: ${(props) => `span ${props.theme.grid.large.columns}`};
      max-width: ${$isRootParent
        ? `${
            theme.grid.large.maxWidth
