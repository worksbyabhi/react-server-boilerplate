...
export const GridColumn = styled.div<GridColumnProps>`
  grid-column: ${(props) => `span ${props.$spanSmall || props.theme.grid.small.columns}`};
  display: ${(props) => (props.$spanSmall === 0 ? 'none' : 'flex')};
  flex-direction: column;
  align-items: ${(props) => props.$alignItemsSmall ?? 'flex-start'};

  ${({ theme, $spanMedium, $spanDesktop, $alignItemsMedium, $alignItemsDesktop }) => css`
    @media screen and (min-width: ${`${theme.breakpoints.medium}px`}) {
      grid-column: ${(props) =>
        `span ${$spanMedium || $spanDesktop || props.theme.grid.medium.columns}`};
    ...
    @media screen and (min-width: ${`${theme.breakpoints.large}px`}) {
      grid-column: ${(props) => `span ${$spanDesktop || props.theme.grid.large.columns}`};
